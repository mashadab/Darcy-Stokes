...
def build_grid3D(Grid):
    ...
    #In z-direction
    if not hasattr(Grid,'zmin'):
        Grid.zmin = 0
        print("Grid.zmin is not defined and has been set to 0.\n")
    if not hasattr(Grid,'zmax'):
        Grid.zmax = 1 
        print("Grid.zmax is not defined and has been set to 1.\n")
    if not hasattr(Grid,'Nz'): 
        Grid.Nz   = 1
        print("Grid.Nz is not defined and has been set to 1.\n")
    ...
    Grid.zc = np.transpose(np.linspace(Grid.zmin+Grid.dz/2, Grid.zmax-Grid.dz/2, Grid.Nz))
    ...
    Grid.zf = np.transpose(np.linspace(Grid.zmin, Grid.zmax, Grid.Nz+1)) # z-coords of gridblock faces
