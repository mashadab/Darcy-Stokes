function [A] = flux_upwind2D(q,Grid) % repo
% author: 
% date: 
% Description:
...
elseif (Nx>1) && (Ny>1) % 2D
    % x-matrices
     ...
     % y-matrices
     Ix = speye(Nx);
     Ayp1 = spdiags(ones(Ny,1),-1,Ny+1,Ny);  % 1D x-poititve
     Ayn1 = spdiags(ones(Ny,1), 0,Ny+1,Ny);   % 1D x-negative
     Ayp  = kron(Ix,Ayp1);                    % 2D x-positive
     Ayn  = kron(Ix,Ayn1);                    % 2D x-negative
