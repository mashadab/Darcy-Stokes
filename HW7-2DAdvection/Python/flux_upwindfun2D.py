...
def flux_upwind(q,Grid):
    ...
    if (Nx>1) and (Ny>1): #2D case
        ...
        Axp1 = sp.spdiags(([np.array(np.ones((Nx+1),'float64'))]),np.array([-1]),Nx+1,Nx).toarray() # Axp^1
        Axn1 = sp.spdiags(([np.array(np.ones((Nx+1),'float64'))]),np.array([0]),Nx+1,Nx).toarray() # Axn^1
        ...
        # y -fluxes       
        Ix = (sp.eye(Nx)).toarray()
        
        Ayp1 = sp.spdiags(([np.array(np.ones((Ny+1),'float64'))]),np.array([-1]),Ny+1,Ny).toarray() # Ayp^1
        Ayn1 = sp.spdiags(([np.array(np.ones((Ny+1),'float64'))]),np.array([0]),Ny+1,Ny).toarray() # Ayn^1
        ...
        A    = sp.spdiags(([qp,qn]),np.array([-1,0]),Nx+1,Nx).toarray()

    elif (Nx==1) and (Ny>1): #2D case    
        qn   = np.minimum(q[0:N,0],0)
        qp   = np.maximum(q[1:N+1,0],0)
        A    = sp.spdiags(([qp,qn]),np.array([-1,0]),N+1,N).toarray()
