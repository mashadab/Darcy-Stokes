function [phiD,Av] = evolve_porosity(D,I,phiD,vD,pD,B,N,BC,Grid,phi_c,theta,dtD)
% author: Marc Hesse
% date: 16 Apr 2019
...
Av = flux_upwind2D(vD,Grid);
L =  phi_c * D * Av - spdiags(pD,0,Grid.N,Grid.N);
IM = I + dtD*(1-theta)*L;
EX = I - dtD*theta*L;
phiD = solve_lbvp(IM,EX*phiD,B,BC.g,N);


if max(phiD*phi_c) > 1 || min(phiD*phi_c) < 0; error('Porosity outside  [0 1].\n'); end